...
## v17 Builder Blueprint
- Canonical desktop layout: sticky synergy readout, vertical stats rail, independently scrolling editor.
- Dedicated mobile portrait and landscape shells; only one shell is mounted at a time.
- Compact interactive stats rail, collapsible calculation breakdown, and build toolbar.


## v17 — Equipment editor and authoritative weapon damage types
- Exotic weapon element labels now prefer exact equipped/owned manifest data.
- Selecting a curated exotic validates its current damage type and slot against Bungie's InventoryItemDefinition.
- Manifest cache was invalidated to remove stale element mappings.
- Corrected known curated mismatches including Hawkmoon, Coldheart, Final Warning, Delicate Tomb, Choir of One, and Wicked Implement.


## v17 — Always-visible equipment editing

- Primary, Secondary, Heavy, Helmet, Gauntlets, Chest, Legs, and Class Item remain visible in the Builder.
- Weapon rarity filtering is unified into All / Exotic / Legendary per slot.
- Owned and curated Exotic weapons share the same slot selector.
- Synced armor immediately exposes planned capacity, masterwork, and mod controls.
- Real armor mods seed the editable plan after selection; planned changes never silently alter the live character.
- Removed the stale instruction to press Sync My Real Build after account data is already available.
- Detailed perk/mod editors remain collapsed until requested to avoid building hundreds of controls during the normal Builder render.


## v18 consolidation
See RELEASE_NOTES_v18.txt.
