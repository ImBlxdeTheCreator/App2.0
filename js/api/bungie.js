...
/* =========================================================================
   ...
   ========================================================================= */
const D2S_CONFIG = {
  MANIFEST_CACHE_VERSION: "v7-authoritative-weapon-elements",
...
async function resolvePublicBungieIcon(source, itemType){
  ...
  if(Object.prototype.hasOwnProperty.call(iconCache, cacheKey) && iconCache[cacheKey]) return iconCache[cacheKey];
  if(Object.prototype.hasOwnProperty.call(iconCache, cacheKey) && !iconCache[cacheKey]) delete iconCache[cacheKey];
...
function attachLiveIcon(container, source, itemType, options={}){
  ...
  const showFallback=()=>{
    ...
    img.remove();
    if(container.querySelector('.bungieIconFallback'))return;
    ...
    fallback.textContent=(parts.name||'?').split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'?';
    fallback.title=`Image unavailable for ${parts.name||'this item'}`;container.insertBefore(fallback,container.firstChild);
  };
  const load = async()=>{
    if(started) return; started = true;
    let url = await resolvePublicBungieIcon(source, itemType);
    if(!img.isConnected) return;
    if(!url){ showFallback(); return; }
    let retried=false;
    img.onerror = async()=>{
      if(!retried&&parts.name){retried=true;const key=`${String(itemType||'item').toLowerCase()}|${normalizeIconLookupName(parts.name)}`;delete iconCache[key];const retry=await resolvePublicBungieIcon(parts.name,itemType);if(retry&&retry!==img.src){img.src=retry;return;}}
      showFallback();
    };
    img.src = url;
