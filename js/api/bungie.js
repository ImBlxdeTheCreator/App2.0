...
function renderSyncPanel(){
  ...
  col.innerHTML = '';
  const addRow=(label,handler,testid)=>{
    const row=el('button','drawerRow');row.type='button';if(testid)row.dataset.testid=testid;
    row.appendChild(el('span','rowLabel',label));row.appendChild(el('span','rowChev','&#10095;'));
    row.onclick=handler;col.appendChild(row);return row;
  };
  addRow('Sign in with Bungie',()=>{
    const auth=getStoredAuth();
    if(auth){document.getElementById('globalSyncBtn')?.click();closeDrawer();}
    else startBungieSignIn();
  },'signin-bungie-menu');
  addRow('Go Offline',()=>{
    clearStoredAuth();state.selectedCharId=null;liveSyncState.characters=null;
    localStorage.setItem('d2synergy_start_mode','offline');closeDrawer();render();
  },'go-offline-menu');
  addRow('Settings',()=>{closeDrawer();window.D2Settings?.open?.();},'open-settings-menu');
  const footerCol=document.getElementById('syncDrawerFooter');footerCol.innerHTML='';
  const status=el('div','empty-note',getStoredAuth()?'Bungie account connected.':'Offline mode or Bungie sign-in available.');
  footerCol.appendChild(status);
}
renderSyncPanel();
