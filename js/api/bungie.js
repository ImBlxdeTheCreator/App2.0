...
/* =========================================================================
   ...
   ========================================================================= */
const D2S_CONFIG = {
  MANIFEST_CACHE_VERSION: "v8-unified-destiny-assets",
...
function setStoredAuth(data){
  try{localStorage.setItem(OAUTH_STORAGE_KEY, JSON.stringify(data));}catch(e){throw new Error('This browser blocked local account storage. Allow site data for D2Synergy and try again.');}
}
function clearStoredAuth(){
  try{localStorage.removeItem(OAUTH_STORAGE_KEY);}catch(e){}
}

function startBungieSignIn(){
  const state = Math.random().toString(36).slice(2) + Date.now().toString(36);
  try{sessionStorage.setItem("d2synergy_oauth_state", state);}catch(e){}
...
// Resolves a weapon by exact display name, then loads the current Bungie
// InventoryItemDefinition. This is used for damage-type validation; curated
// builder data is never allowed to override the manifest when a definition is
// available. Results share the persistent manifest cache.
const __d2sWeaponDefinitionInFlight = new Map();
async function resolveAuthoritativeWeaponDefinitionByName(name){
  const lookupName=String(name||'').trim();
  const normalizedName=normalizeIconLookupName(lookupName);
  if(!normalizedName) return null;
  const cacheKey=`weapon-definition|${normalizedName}`;
  if(Object.prototype.hasOwnProperty.call(iconCache,cacheKey)) return iconCache[cacheKey]||null;
  if(__d2sWeaponDefinitionInFlight.has(cacheKey)) return __d2sWeaponDefinitionInFlight.get(cacheKey);
  const request=(async()=>{
    try{
      const res=await rateLimitedFetch(`${BUNGIE_BASE}/Destiny2/Armory/Search/DestinyInventoryItemDefinition/${encodeURIComponent(lookupName)}/`,{headers:{'X-API-Key':BUNGIE_API_KEY}});
      ...
      const ranked=results.map(r=>{
        ...
        score+=iconTypeScore(r,'weapon');
        if(r?.redacted) score-=50;
        return {r,score};
      }).sort((a,b)=>b.score-a.score);
      const best=ranked[0];
      if(!best||best.score<80||!best.r?.hash) throw new Error('no exact weapon match');
      const def=await getItemDefinition(best.r.hash);
      const compact=def?{hash:def.hash,name:def.name,icon:def.icon,damageType:def.damageType,bucketHash:def.bucketHash,itemType:def.itemType,tierType:def.tierType,typeName:def.typeName}:null;
      iconCache[cacheKey]=compact; persistIconCache();
      return compact;
    }catch(e){
      iconCache[cacheKey]=null; persistIconCache();
      return null;
    }finally{__d2sWeaponDefinitionInFlight.delete(cacheKey);}
  })();
  __d2sWeaponDefinitionInFlight.set(cacheKey,request);
  return request;
}
function manifestWeaponElement(def){
  return def&&DAMAGE_TYPE_TO_ELEMENT[Number(def.damageType)]||null;
}

// Shared icon helper. The resolver is loaded with the Builder so account,
// offline, Vault, Activities, and Collections all use the same pipeline.
function attachLiveIcon(container, source, itemType, options={}){
  return window.D2Assets?.attach(container, source, itemType, options) || null;
}
...
async function selectRealArmor(slotName, item){
  ...
  try {
    ...
    // Seed the editable planned-build controls from the synced piece instead of
    // hiding them behind a second sync prompt. This never changes the live item.
    const regular = item.matchedMods.filter(m => (ARMOR_MODS_BY_SLOT[slotName]||[]).some(k=>k.name===m.name)).slice(0,4).map(m=>m.name);
    const tuning = item.matchedMods.find(m => (TUNING_MODS_BY_SLOT[slotName]||[]).some(k=>k.name===m.name));
    state.mods[slotName] = regular;
    state.tuning[slotName] = tuning ? tuning.name : null;
    state.armorTier[slotName] = Math.max(1, Math.min(5, Number(item.energyCapacity||10) >= 11 ? 4 : 3));
    state.armorMasterwork[slotName] = {level:item.isMasterworked ? 5 : 0, stats:[]};
    item.unmatchedPlugNames = (item.socketedPlugHashes||[])
      .map(h=>resolvedPlugDefs[h]?.name)
  ...
  __defInFlight[hash] = (async ()=>{
    try {
      ...
      const result = {
        ...
        secondaryIcon: def.secondaryIcon || def.displayProperties?.secondaryIcon ? publicBungieIconUrl(def.secondaryIcon || def.displayProperties?.secondaryIcon) : null,
        screenshot: def.screenshot ? publicBungieIconUrl(def.screenshot) : null,
        ...
        iconWatermarkShelved: def.iconWatermarkShelved ? "https://www.bungie.net" + def.iconWatermarkShelved : null,
      };
...
async function getFullInventory(membershipType, membershipId){
  ...
  const mapItem = (i, source, characterId=null) => {
    ...
    return {
      ...
      isMasterworked: !!(Number(i.state || 0) & MASTERWORK_STATE_BIT),
...
async function getMembershipsForCurrentUser(accessToken){
  ...
  const primary = memberships.find(m=>String(m.membershipId)===primaryId)
    ...
    || memberships[0];
  const displayName=primary.bungieGlobalDisplayName || primary.displayName || 'Guardian';
  const displayCode=primary.bungieGlobalDisplayNameCode!=null?String(primary.bungieGlobalDisplayNameCode).padStart(4,'0'):'';
  const accountLabel=displayCode?`${displayName}#${displayCode}`:displayName;
  try{localStorage.setItem('d2synergy_account_label',accountLabel);}catch(e){}
  setTimeout(()=>{try{renderSyncPanel();window.dispatchEvent(new CustomEvent('d2synergy:accountchange'));}catch(e){}},0);
  return {
    membershipType: Number(primary.membershipType),
    membershipId: String(primary.membershipId),
    displayName,displayNameCode:displayCode,accountLabel,
      ...
      items.forEach(async (item)=>{
        try {
          ...
          if(def.icon){
            ...
            D2Assets.setImage(img,def,def.itemType===2?'armor':'weapon',{alt:def.name}); img.style.width='32px'; img.style.height='32px'; img.style.borderRadius='4px';
      ...
      function renderGrouped(){
        ...
        orderedKeys.forEach(bucketHash=>{
          ...
          group.forEach(({item,def})=>{
            ...
            if(def.icon){
              const img = document.createElement('img');
              D2Assets.setImage(img,def,def.itemType===2?'armor':'weapon',{alt:def.name}); img.style.width='36px'; img.style.height='36px'; img.style.borderRadius='4px';
              chip.appendChild(img);
            }
...
function renderSyncPanel(){ window.D2AccountMenu?.refresh?.(); }
window.getStoredAuth=getStoredAuth;
window.startBungieSignIn=startBungieSignIn;
window.D2AccountActions={
  signIn(){startBungieSignIn();},
  signOut(){clearStoredAuth();try{localStorage.removeItem('d2synergy_account_label');}catch(e){}state.selectedCharId=null;liveSyncState.characters=null;renderSyncPanel();render();closeDrawer();window.dispatchEvent(new CustomEvent('d2synergy:accountchange'));},
  goOffline(){
    // Preserve the Bungie session and the last synced Guardian. Offline mode
    // disables live refreshes; Sign Out is the explicit credential-removal
    // action.
    ...
    renderSyncPanel();render();closeDrawer();
  ...
  refresh(){renderSyncPanel();}
};
renderSyncPanel();

const syncDrawer = document.getElementById('syncDrawer');
const drawerOverlay = document.getElementById('drawerOverlay');
function openDrawer(){ if(window.D2AccountMenu)return window.D2AccountMenu.open(); syncDrawer?.classList.add('open');drawerOverlay?.classList.add('open'); }
function closeDrawer(){ if(window.D2AccountMenu)return window.D2AccountMenu.close(); syncDrawer?.classList.remove('open');drawerOverlay?.classList.remove('open'); }
if(!window.D2AccountMenu){
...
if(!window.D2AccountMenu)(function(){
...
queueMicrotask(()=>window.D2Assets?.retryAll?.());
