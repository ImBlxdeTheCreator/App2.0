...
(function(){
  ...
  const BUILD='20260716-phase1-19-1-7';
  const HOST='https://www.bungie.net';
  const API_BASE=HOST+'/Platform';
  const API_KEY='cb3731fc41f1468fadab053d31d07abc';
  const SUCCESS_KEY='d2synergy_asset_success_v19_1_7';
  const RESOLUTION_KEY='d2synergy_asset_resolution_v19_1_7';
  ...
  let nextRequestAt=0;
  let registry={version:'19.1.7',definitionTypes:{...DEFAULT_TYPES},fallbacks:{},overrides:{}};
    ...
    const task=(async()=>{
      try{
        ...
        if(ranked[0]?.score>=80){
          ...
          if(url){resolved[key]=url;persist();return url;}
        }
      }catch(_){ }
      return null;
