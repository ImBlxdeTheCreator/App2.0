...
(function(){
  ...
  function unlock(mode){
    try{sessionStorage.setItem('d2synergy_launch_mode',mode);localStorage.setItem('d2synergy_start_mode',mode);}catch(e){}
    document.documentElement.dataset.connectionMode=mode;
  ...
  async function launchOffline(){
    ...
    try{
      ...
      unlock('offline');
      window.D2Status?.set('builder','offline','Local build data');
  ...
  async function launchBungie(){
    ...
    try{
      ...
      setStatus('Account services ready. Syncing your Guardian in the background…');
      unlock('bungie');
      if(typeof syncMyLiveBuildExact==='function'){
        if(window.D2Loader)D2Loader.show('Syncing Guardian','You can use the app while exact equipment and account data refresh.',{blocking:false});
        window.D2Status?.refreshing('builder','Syncing Guardian');
        Promise.resolve(syncMyLiveBuildExact()).then(()=>window.D2Status?.set('builder','live','Guardian build synced')).catch(error=>{
          console.error(error);
          window.dispatchEvent(new CustomEvent('d2synergy:syncerror',{detail:{message:error?.message||'Guardian sync failed'}}));
        }).finally(()=>window.D2Loader?.hide(true));
      }
    }catch(e){
      if(window.D2Loader)D2Loader.hide(true);
      ...
      try{
        ...
        const timer=setInterval(async()=>{
          ...
          if(typeof getStoredAuth==='function'&&getStoredAuth()){
            ...
            try{if(typeof syncMyLiveBuildExact==='function'){window.D2Status?.refreshing('builder','Syncing Guardian');await syncMyLiveBuildExact();window.D2Status?.set('builder','live','Guardian build synced');}}catch(e){console.error(e);window.D2Status?.fail('builder',e.message||'Guardian sync failed');}
            if(window.D2Loader)D2Loader.hide(true);
          }else if(attempts>60){
  ...
  if('serviceWorker' in navigator){
    window.addEventListener('load',async()=>{
      try{
        const reg=await navigator.serviceWorker.register('./service-worker.js?v=19.1.1',{scope:'./',updateViaCache:'none'});
        ...
        reg.addEventListener('updatefound',()=>{
          ...
          worker.addEventListener('statechange',()=>{
            if(worker.state==='installed'&&navigator.serviceWorker.controller){
              // Activate the fully version-matched release. controllerchange
              // below performs one guarded reload, eliminating hard refreshes.
              worker.postMessage({type:'SKIP_WAITING'});
            }
          });
        });
        navigator.serviceWorker.addEventListener('controllerchange',()=>{
          const key='d2synergy_controller_reload_19_1_0';
          try{if(sessionStorage.getItem(key))return;sessionStorage.setItem(key,'1');}catch(e){}location.reload();
        });
