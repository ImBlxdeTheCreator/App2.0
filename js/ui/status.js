...
(function(){
  ...
  function render(scope){
    ...
    el.innerHTML=`<span class="workspaceStatusDot" aria-hidden="true"></span><strong>${labels[value.state]||value.state}</strong>${value.detail?`<span> · ${String(value.detail)}</span>`:''}<time>${stamp}</time>`;
    el.title=value.updatedAt?`Last updated ${new Date(value.updatedAt).toLocaleString()}`:(value.detail||'');
  }
