...
const PANEL_LABELS = {
  ...
  p4:"Tiered Legendary Weapons", p5:"Armor Mods", p6:"Seasonal Artifact", p7:"Legendary Armor Set Bonus",
};
const panelCollapsed = {p1:false,p2:true,p3:true,p4:true,p5:true,p6:true,p7:true};
...
function clearPanel(id){
  switch(id){
    ...
    case 'p7': state.armorSetByPiece={Helmet:null,Arms:null,Chest:null,Legs:null,ClassItem:null}; break;
  }
  render();
}
...
function renderAlwaysVisibleEquipment(root){
  ...
  ['Kinetic','Energy','Power'].forEach(slot=>{
    ...
    const edit=el('button','equipmentEdit','Advanced perks &amp; mods'); edit.type='button'; edit.onclick=()=>openPanel(activeExotic?'p3':'p4'); card.appendChild(edit);
  ...
  ['Helmet','Arms','Chest','Legs','ClassItem'].forEach(slot=>{
    ...
    const controls=el('div','armorQuickControls');
    const cap=el('label','',`Capacity <select aria-label="${armorLabels[slot]} planned capacity"></select>`); const capSel=cap.querySelector('select');
    [10,11].forEach(v=>{const o=new Option(String(v),String(v));if((state.armorTier[slot]>=4?11:10)===v)o.selected=true;capSel.appendChild(o);}); capSel.onchange=()=>{state.armorTier[slot]=Number(capSel.value)===11?4:3;render();}; controls.appendChild(cap);
    const mw=el('label','',`Masterwork <select aria-label="${armorLabels[slot]} planned masterwork"></select>`); const mwSel=mw.querySelector('select');
    [0,1,2,3,4,5].forEach(v=>{const o=new Option(v?'Level '+v:'None',String(v));if((state.armorMasterwork[slot]?.level||0)===v)o.selected=true;mwSel.appendChild(o);}); mwSel.onchange=()=>{state.armorMasterwork[slot].level=Number(mwSel.value);render();}; controls.appendChild(mw); card.appendChild(controls);
    const modSummary=el('div','equipmentModSummary'); const mods=(state.mods[slot]||[]).filter(Boolean); modSummary.textContent=mods.length?mods.join(' · '):(real?.matchedMods?.length?real.matchedMods.map(m=>m.name).join(' · '):'No planned mods'); card.appendChild(modSummary);
    const edit=el('button','equipmentEdit','Edit mods &amp; tuning');edit.type='button';edit.onclick=()=>openPanel('p5');card.appendChild(edit);
...
function renderBuilder(){
  ...
  const clsRow = el('div','row');
  const clsSel = el('select');
  ["Titan","Hunter","Warlock"].forEach(c=>{
    const o=document.createElement('option'); o.value=c; o.textContent=c; if(c===state.cls) o.selected=true; clsSel.appendChild(o);
  });
  ...
  const liveFragmentObjects=state.liveSubclass?.fragments||[];
  ...
  // Artifact
  try {
  const {header: p6Head, body: p6} = makePanel('p6', '<span class="n">06</span> Seasonal Artifact', {skipPanelClear:true});
  root.appendChild(p6Head);
  ...
  if(state.artifactMode === "real"){
    if(!getStoredAuth()){
      ...
      if(state.artifact){
        ...
        p6.appendChild(el('div','empty-note', 'Use "Sync My Real Build" at the top to pull your account\u2019s currently active artifact perks.'));
      }
      root.appendChild(p6);
    }
  } else {
  ...
  } catch(e){ console.error('Panel 6 (Artifact) render failed:', e); root.appendChild(el('div','panel', '<div style="color:var(--danger);padding:8px;">Error rendering Seasonal Artifact: '+e.message+'</div>')); }

  // Armor Set Bonus — one dropdown per armor piece, since Armor 3.0 lets
  // each piece carry a different set's bonus independently.
  try {
  const {header: p7Head, body: p7} = makePanel('p7', '<span class="n">07</span> Legendary Armor Set Bonus', {skipPanelClear:true});
  root.appendChild(p7Head);

  const setModeRow = el('div','subclasstabs');
  setModeRow.style.marginBottom = '8px';
  [{v:"generic",label:"Generic"},{v:"real",label:"My Real Live Build"}].forEach(opt=>{
    const b = document.createElement('button');
    b.textContent = opt.label;
    if(state.armorSetMode === opt.v) b.classList.add('active');
    b.onclick = ()=>{ state.armorSetMode = opt.v; render(); };
    setModeRow.appendChild(b);
  });
  setModeRow.appendChild(makeItemClearBtn(()=>{
    state.armorSetByPiece = {Helmet:null,Arms:null,Chest:null,Legs:null,ClassItem:null};
    state.armorSetMode = "generic";
  }));
  p7.appendChild(setModeRow);

  if(state.armorSetMode === "real"){
    if(!getStoredAuth()){
      ...
      if(livePieces.length){
        ...
        livePieces.forEach(([piece,item])=>{
          ...
          const liveSet=state.liveArmorSetByPiece?.[piece];
          if(liveSet?.name) text.appendChild(el('span','liveManifestIconLabel',`Set bonus: ${liveSet.name}`));
      ...
      const liveSetCounts={};
      Object.values(state.liveArmorSetByPiece||{}).forEach(info=>{if(info?.name) liveSetCounts[info.name]=(liveSetCounts[info.name]||0)+1;});
      Object.entries(liveSetCounts).forEach(([name,count])=>{
        const note=el('div','empty-note',`<b style="color:var(--gold);font-style:normal;">${name}</b> — ${count} equipped piece${count===1?'':'s'} detected from exact socket plugs.`);
        note.style.marginTop='6px'; p7.appendChild(note); attachLiveIcon(note,{name},'perk');
      });
      const hasAny = Object.values(state.armorSetByPiece).some(v=>v) || Object.values(state.liveArmorSetByPiece||{}).some(v=>v?.name);
      if(!hasAny) p7.appendChild(el('div','empty-note', 'No armor set-bonus plug was returned on the selected character’s equipped armor.'));
    }
  } else {
  const setPieceLabels = {Helmet:"Helmet", Arms:"Arms", Chest:"Chest", Legs:"Legs", ClassItem:"Class Item"};
  ["Helmet","Arms","Chest","Legs","ClassItem"].forEach(piece=>{
    const row = el('div','slotrow');
    row.appendChild(el('label','',setPieceLabels[piece]));
    const sel = el('select');
    const noneOpt = document.createElement('option'); noneOpt.value=""; noneOpt.textContent="— none —"; sel.appendChild(noneOpt);
    ARMOR_SETS.forEach(s=>{
      const o=document.createElement('option'); o.value=s.name; o.textContent=`${s.name} (${s.category})`;
      if(state.armorSetByPiece[piece]===s.name) o.selected=true;
      sel.appendChild(o);
    });
    sel.onchange=()=>{ state.armorSetByPiece[piece] = sel.value || null; render(); };
    row.appendChild(sel);
    p7.appendChild(row);
  });
  }
  ...
  Object.values(state.armorSetByPiece).forEach(setName=>{
    if(setName) setCountsUI[setName] = (setCountsUI[setName]||0) + 1;
  });
  const activeSetNames = Object.keys(setCountsUI);
  if(activeSetNames.length){
    activeSetNames.forEach(setName=>{
      const count = setCountsUI[setName];
      const set = ARMOR_SETS.find(s=>s.name===setName);
      if(!set) return;
      const setNote = el('div','empty-note',
        `<b style="color:var(--gold);font-style:normal;">${setName}</b> — ${count} piece${count===1?'':'s'} equipped<br>`+
        `<span style="color:${count>=2?'var(--text)':'var(--muted2)'};">${set.twoPiece.name} (2pc)${count>=2?' \u2713':' \u2014 need 2'}: ${set.twoPiece.effects.map(e=>BUCKETS.find(b=>b.id===e.bucket).name+' +'+e.value+'%').join(', ')}</span><br>`+
        `<span style="color:${count>=4?'var(--text)':'var(--muted2)'};">${set.fourPiece.name} (4pc)${count>=4?' \u2713':' \u2014 need 4'}: ${set.fourPiece.effects.map(e=>BUCKETS.find(b=>b.id===e.bucket).name+' +'+e.value+'%').join(', ')}</span>`
      );
      setNote.style.marginTop='8px';
      p7.appendChild(setNote);
      attachLiveIcon(setNote, count>=4 ? set.fourPiece.name : set.twoPiece.name, 'perk');
    });
    const mixNote = el('div','empty-note', 'Each piece can carry a different set\u2019s bonus \u2014 multiple sets can be active at once if you spread pieces across them, matching how Armor 3.0 set mods actually work.');
    mixNote.style.marginTop='8px';
    p7.appendChild(mixNote);
  }
  root.appendChild(p7);
  } catch(e){ console.error('Panel 7 (Set Bonus) render failed:', e); root.appendChild(el('div','panel', '<div style="color:var(--danger);padding:8px;">Error rendering Legendary Armor Set Bonus: '+e.message+'</div>')); }
}
