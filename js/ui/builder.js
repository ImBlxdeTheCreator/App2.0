...
const PANEL_LABELS = {
  ...
  p4:"Tiered Legendary Weapons", p5:"Armor Mods", p6:"Seasonal Artifact",
};
const panelCollapsed = {p1:false,p2:true,p3:true,p4:true,p5:true,p6:true};
...
function renderAlwaysVisibleEquipment(root){
  ...
  ['Helmet','Arms','Chest','Legs','ClassItem'].forEach(slot=>{
    ...
    const setBox=el('div','armorSetInline');
    const setSelect=el('select','equipmentSelect');setSelect.appendChild(new Option('— No armor set bonus —',''));
    (window.ARMOR_SETS||[]).forEach(set=>{const o=new Option(set.name,set.name);if(plannedSet===set.name)o.selected=true;setSelect.appendChild(o);});
    setSelect.onchange=()=>{state.armorSetByPiece[slot]=setSelect.value||null;render();};setBox.appendChild(setSelect);
    if(setName){const counts={};Object.values({...state.armorSetByPiece,...Object.fromEntries(Object.entries(state.liveArmorSetByPiece||{}).map(([k,v])=>[k,v?.name]))}).forEach(n=>{if(n)counts[n]=(counts[n]||0)+1;});const count=counts[setName]||1;const def=(window.ARMOR_SETS||[]).find(x=>x.name===setName);setBox.appendChild(el('div','equipmentModSummary',def?`${setName} · ${count} piece${count===1?'':'s'} · ${count>=4?'4-piece active':count>=2?'2-piece active':`need ${2-count} more for 2-piece`}`:`${setName} · ${count} piece${count===1?'':'s'}`));}
