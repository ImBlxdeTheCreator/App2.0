...
function buildEquipmentEditorSummary(kind,slot){
  ...
  if(kind==='armor'){
    ...
    const champ=window.D2ChampionCapability?.(real?._def||current,[]);if(champ)facts.appendChild(el('div','equipmentInspectorFact',`<span>Champion capability</span><b>${champ.effect}</b>`));
    const origin=selected?.originTrait||real?.originTrait?.name||null;if(origin)facts.appendChild(el('div','equipmentInspectorFact wide',`<span>Origin Trait</span><b>${origin}</b>`));
    const normalPerks=[...new Set(known.filter(name=>name!==origin))];if(normalPerks.length)facts.appendChild(el('div','equipmentInspectorFact wide',`<span>Weapon perks</span><b>${normalPerks.join(' · ')}</b>`));
  }
...
function renderAlwaysVisibleEquipment(root){
  ...
  ['Kinetic','Energy','Power'].forEach(slot=>{
    ...
    const edit=el('button','equipmentEdit','Perks, stats &amp; mods'); edit.type='button';
    edit.onclick=()=>{
      const inspected=state.legendaryRealItem?.[slot]||state.liveExoticWeapon||null;
      if(inspected&&typeof openItemDetail==='function')openItemDetail(inspected,{source:'builder',slot});
      else openEquipmentEditor('weapon',slot,activeExotic?'p3':'p4');
  ...
  ['Helmet','Arms','Chest','Legs','ClassItem'].forEach(slot=>{
    ...
    const setStatus=setName?(setDef?`${setName} · ${count}/4 · ${count>=4?'4-piece active':count>=2?'2-piece active · 4-piece inactive':`inactive · need ${2-count} more for 2-piece`}`:`${setName} · ${count} piece${count===1?'':'s'} · threshold data unavailable`):'No armor set on this piece';
    const capacity=Number(real?.energyCapacity ?? (state.armorTier[slot]>=4?11:10));
    ...
    const edit=el('button','equipmentEdit','Stats, perks &amp; mods');edit.type='button';
    edit.onclick=()=>{
      const inspected=state.armorRealItem?.[slot]||null;
      if(inspected&&typeof openItemDetail==='function')openItemDetail(inspected,{source:'builder',slot});
      else openEquipmentEditor('armor',slot,'p5');
    };card.appendChild(edit);
