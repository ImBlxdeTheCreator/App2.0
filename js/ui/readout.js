...
function renderReadout(){
  ...
  const findings=[];
  if(statTierOf('Resilience')<7)findings.push({level:'warn',text:`Resilience is Tier ${statTierOf('Resilience')}. Harder activities may expose this survivability tradeoff.`});
  if(statTierOf('Recovery')<5)findings.push({level:'warn',text:`Recovery is Tier ${statTierOf('Recovery')}. Health regeneration may feel less forgiving.`});
