...
function renderReadout(){
  ...
  const findings=[];
  if(statTierOf('Health')<7)findings.push({level:'warn',text:`Health is Tier ${statTierOf('Health')}. Harder activities may expose this survivability tradeoff.`});
  if(statTierOf('Class')<5)findings.push({level:'warn',text:`Class is Tier ${statTierOf('Class')}. Recovery and class-ability performance may feel less forgiving.`});
