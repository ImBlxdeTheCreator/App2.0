...
async function openItemDetail(it,context){
  ...
  if(originGroups.length){const origin=dimText('section','dimSection dimOriginSection');origin.appendChild(dimText('div','dimSectionTitle','Origin Traits'));const matrix=dimText('div','dimSocketMatrix');originGroups.forEach(g=>matrix.appendChild(dimBuildSocketColumn(g,inspector)));origin.appendChild(matrix);overview.appendChild(origin);}
