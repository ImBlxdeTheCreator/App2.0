...
function dimIcon(src,cls,alt){const img=document.createElement('img');if(cls)img.className=cls;img.src=src;img.alt=alt||'';img.loading='lazy';img.onerror=()=>img.classList.add('iconFailed');return img;}
...
async function dimBuildSocketGroups(it,itemDef,token){
  ...
  return Promise.all(visible.map(async(socket,index)=>{
    ...
    candidateHashes=Array.from(new Set(candidateHashes.filter(Boolean))).slice(0,utility?1:36);
...
function dimBuildSocketColumn(group,inspector){
  const col=dimText('div','dimSocketColumn');col.dataset.category=group.category;
...
async function openItemDetail(it,context){
  ...
  const meta=[def.typeName||fsBucketLabel(it)];const dmg=FS_DMG[def.damageType];if(dmg)meta.push(dmg.n);if(def.tierType===6)meta.push('Exotic');if(Number(it.quantity||1)>1)meta.push(`Quantity ${Number(it.quantity).toLocaleString()}`);
  ...
  const perkSection=dimText('section','dimSection dimSocketSection');perkSection.appendChild(dimText('div','dimSectionTitle','Perks'));
  if(coreGroups.length){const matrix=dimText('div','dimSocketMatrix');coreGroups.forEach(g=>matrix.appendChild(dimBuildSocketColumn(g,inspector)));perkSection.appendChild(matrix);const selected=coreGroups.flatMap(g=>g.choices).find(c=>c.selected)||coreGroups[0].choices[0];dimRenderPlugInspector(inspector,selected);perkSection.appendChild(inspector);}else{perkSection.appendChild(dimText('div','dimNoSockets','No perk sockets were returned for this item.'));}
