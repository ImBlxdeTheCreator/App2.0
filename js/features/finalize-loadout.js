...
// Single character selector: live emblem cards with class and Power.
// Class/subclass editing remains inside the Build Editor, so this bar never
// creates duplicate class or subclass controls.
function renderCharBar(){
  const bar=document.getElementById('charBar');
  if(!bar)return;
  bar.replaceChildren();
  bar.appendChild(el('span','cbLabel','Guardian'));
  const main=el('div','charBarMain');
  const auth=getStoredAuth();
  if(auth&&liveSyncState.characters?.length){
    const rail=el('div','charCardRail');
    liveSyncState.characters.forEach(c=>{
      const selected=String(state.selectedCharId)===String(c.charId);
      const card=el('button','charCard'+(selected?' active':''));
      card.type='button';card.setAttribute('data-testid','charbar-live-'+c.charId);card.setAttribute('aria-pressed',selected?'true':'false');
      const className=CLASS_TYPE_NAMES[c.classType]||'Guardian';
      if(c.emblemPath){
        const image=document.createElement('img');image.className='cc-emblem';image.alt='';
        D2Assets.setImage(image,{name:`${className} emblem`,icon:c.emblemPath},'emblem',{lazy:false});card.appendChild(image);
      }
      card.appendChild(el('span','charCardCopy',`<span class="cc-cls">${className}</span><span class="cc-pw">◈ ${c.light||'?'}</span>`));
      card.onclick=()=>selectLiveCharacter(c.charId);rail.appendChild(card);
    });
    main.appendChild(rail);
    if(state.selectedCharId){const actions=el('div','charBarActions');const button=el('button','reset','Finalize Loadout ▸');button.type='button';button.setAttribute('data-testid','open-finalize-btn');button.style.borderColor='var(--good)';button.style.color='var(--good)';button.onclick=openFinalizeModal;actions.appendChild(button);main.appendChild(actions);}
  }else if(auth){
    const actions=el('div','charBarActions');const button=el('button','reset',liveSyncState.loading?'Loading…':'Load my characters');button.type='button';button.setAttribute('data-testid','charbar-load-btn');button.disabled=liveSyncState.loading;button.onclick=()=>loadLiveCharacters();actions.appendChild(button);main.appendChild(actions);
  }else{
    main.appendChild(el('span','empty-note','Sign in from the menu to load your emblem-based Guardian selector.'));
  }
