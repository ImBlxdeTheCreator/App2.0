...
function classifySubclassSocket(desc){
  ...
  const text=normalizeLiveName([
    desc.categoryDef?.name,desc.categoryDef?.description,
    desc.socketTypeDef?.name,desc.socketTypeDef?.description,
    p.typeName,p.plugCategoryIdentifier,(p.traitIds||[]).join(' '),p.name,
  ].filter(Boolean).join(' '));
  const plugId=String(p.plugCategoryIdentifier||'').toLowerCase();
  if(/aspect/.test(text)||plugId.includes('aspect')) return 'aspect';
  if(/fragment|facet|ember|echo|spark|whisper|thread/.test(text)||plugId.includes('fragment')) return 'fragment';
...
window.__D2SYNERGY_BUILD__='2026.07.14-inventory-stats-5';
window.__D2SYNERGY_SYNC_MODEL__='Exact equipped + carried inventory + Character/Profile Postmaster reconciliation + ProfileCurrencies + CharacterComponent stats';
window.syncMyLiveBuildExact=syncMyLiveBuildExact;
console.info('[D2Synergy] build',window.__D2SYNERGY_BUILD__,'loaded:',window.__D2SYNERGY_SYNC_MODEL__);
