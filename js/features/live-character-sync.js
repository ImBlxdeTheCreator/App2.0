...
function mapSubclassToBuilder(live){
  ...
  state.fragments=live.fragments.map(x=>x.name).filter(name=>name&&!/^(empty|none|unknown|placeholder)/i.test(String(name))&&!/empty (fragment|socket)/i.test(String(name)));
  live.fragments=live.fragments.filter(x=>state.fragments.some(name=>liveNameMatches(name,x.name)));
}
