...
async function loadActivities(){
  ...
  body.innerHTML = '';
  if(window.D2Loader){D2Loader.show('Loading Operations','Fetching public milestones and rewards…');D2Loader.progress(0.12);}
  ...
  try {
    const ms = await bungieGet('/Destiny2/Milestones/');
    if(window.D2Loader){D2Loader.update('Resolving Operations','Loading milestone definitions and rewards…');D2Loader.progress(0.48);}
    ...
    if(window.D2Loader)D2Loader.progress(0.9);
...
function buildActivityCard(c){
  ...
  if(c.icon){ const img=document.createElement('img'); img.src=c.icon; img.alt=c.name; img.loading='lazy'; img.decoding='async'; head.appendChild(img); }
...
async function loadVendors(){
  ...
  try {
    ...
    if(window.D2Loader){D2Loader.update('Loading Vendors','Loading your active Guardian and vendor access…');D2Loader.progress(0.24);}
    ...
    if(window.D2Loader){D2Loader.update('Loading Vendors','Fetching live sales and costs…');D2Loader.progress(0.42);}
    ...
    if(window.D2Loader){D2Loader.update('Rendering Vendors','Preparing item icons, costs, and refresh times…');D2Loader.progress(0.88);}
...
function renderVendors(){
  ...
  (vendorsState.data||[]).forEach(v=>{
    ...
    if(v.icon){ const img=document.createElement('img'); img.src=v.icon; img.loading='lazy'; img.decoding='async'; head.appendChild(img); }
    ...
    items.forEach(it=>{
      ...
      if(it.def.icon){ const im=document.createElement('img'); im.className='ic'; im.src=it.def.icon; im.loading='lazy'; im.decoding='async'; im.fetchPriority='low'; cell.appendChild(im); }
      const info = el('div','');
      info.appendChild(el('div','nm', it.def.name));
      if(it.def.typeName) info.appendChild(el('div','typ', it.def.typeName));
      it.costs.forEach(c=>{
        const cr = el('div','cost');
        if(c.icon){ const ci=document.createElement('img'); ci.src=c.icon; ci.loading='lazy'; ci.decoding='async'; cr.appendChild(ci); }
...
document.getElementById('vendorsSearch').addEventListener('input', (e)=>{
  ...
  __vendorSearchTimer=setTimeout(()=>{vendorsState.search=value;if(vendorsState.data)renderVendors();},90);
});
