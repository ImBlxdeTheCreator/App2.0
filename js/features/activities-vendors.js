...
const PROGRESS_TABS=[
  ['overview','Overview'],['vanguard','Vanguard'],['crucible','Crucible'],['gambit','Gambit'],
  ['raids','Raids'],['dungeons','Dungeons'],['exotic','Exotic Missions'],['monument','Monument'],
  ['patterns','Patterns'],['catalysts','Catalysts'],['triumphs','Triumphs'],['collections','Collections']
];
...
function operationMatches(tab,o){
  ...
  if(tab==='raids')return /raid|pantheon/.test(t);
  if(tab==='dungeons')return /dungeon/.test(t);
  if(tab==='exotic')return /exotic mission|exotic quest|presage|seraph|avalon|whisper|zero hour/.test(t);
  return false;
}
async function loadGuardianProgressProfile(){
  try{
    ...
    const res=await fetch(`${BUNGIE_BASE}/Destiny2/${m.membershipType}/Profile/${m.membershipId}/?components=100,104,200,202,204,500,700,800,900,1100,1200,1300`,{headers:{'X-API-Key':BUNGIE_API_KEY,'Authorization':'Bearer '+auth.access_token}}).then(r=>r.json());
...
async function buildProgressViews(){
  ...
  views.set('gambit',renderOperationProgressView('gambit','Gambit','Current Gambit activities, challenges, and available rewards.'));
  views.set('raids',renderOperationProgressView('raids','Raids','Live raid nodes, weekly challenges, modifiers, and reward previews.'));
  views.set('dungeons',renderOperationProgressView('dungeons','Dungeons','Live dungeon nodes, rotators, modifiers, and reward previews.'));
  views.set('exotic',renderOperationProgressView('exotic','Exotic Missions','Exotic mission nodes and rewards currently exposed by the activity feed.'));
  const monument=progressView('Monument','Monument of Triumph kiosk ownership and visible account entries.');const kioskMaps=[profile?.profileKiosks?.data?.kioskItems,...Object.values(profile?.characterKiosks?.data||{}).map(x=>x?.kioskItems)].filter(Boolean);const kioskCount=kioskMaps.reduce((n,m)=>n+Object.keys(m).length,0);monument.appendChild(el('div','progressEmpty',profile?`${kioskCount} kiosk status entr${kioskCount===1?'y':'ies'} are available for this account. Open Vendors & Kiosks for purchasable inventory and costs.`:'Sign in with Bungie to load kiosk ownership status.'));views.set('monument',monument);
  ...
  const triumphs=progressView('Triumphs','Near-complete, tracked, and completed records from Bungie’s Records component.',profile?'Account data loaded':'Sign in required');if(recordCards.length){const sec=sectionBlock('Closest to completion',`${recordCards.length} records sampled`),grid=el('div','progressCards');recordCards.slice(0,60).forEach(c=>grid.appendChild(progressDataCard(c.name,c.complete?'Complete':c.desc||'In progress',c.pct)));sec.appendChild(grid);triumphs.appendChild(sec);}else triumphs.appendChild(el('div','progressEmpty',profile?'No record entries were returned.':'Sign in with Bungie to load Triumph progress.'));views.set('triumphs',triumphs);
  const collections=progressView('Collections','Account collectible discovery status without guessing ownership.',profile?'Account data loaded':'Sign in required');if(profile){const merged={...(profile.profileCollectibles?.data?.collectibles||{})};Object.values(profile.characterCollectibles?.data||{}).forEach(x=>Object.assign(merged,x?.collectibles||{}));const cards=await resolveCollectibleCards(merged);if(cards.length){const sec=sectionBlock('Recently resolved collection entries',`${cards.length} entries sampled`),grid=el('div','progressCards');cards.forEach(c=>grid.appendChild(progressDataCard(c.name,c.acquired?'Acquired':c.desc||'Not acquired',c.acquired?100:0)));sec.appendChild(grid);collections.appendChild(sec);}else collections.appendChild(el('div','progressEmpty','No collectible status entries were returned.'));}else collections.appendChild(el('div','progressEmpty','Sign in with Bungie to load collection discovery status.'));views.set('collections',collections);
  return views;
}
async function renderActivitiesProgress(){
  const body=document.getElementById('activitiesBody');body.innerHTML='';const shell=el('div','progressShell');const rail=el('nav','progressRail');rail.appendChild(el('div','progressRailTitle','Activities & Progress'));
  const main=el('div','progressMain');const views=await buildProgressViews();
  PROGRESS_TABS.forEach(([id,label])=>{const b=el('button','progressTab');b.type='button';b.dataset.progressTab=id;b.appendChild(el('span','',label));const count=id==='overview'?activitiesProgressState.milestones.length:id==='patterns'?Object.keys(activitiesProgressState.profile?.profileCraftables?.data?.craftables||{}).length:'';if(count!=='')b.appendChild(el('span','progressTabCount',String(count)));b.onclick=()=>{activitiesProgressState.tab=id;rail.querySelectorAll('.progressTab').forEach(x=>x.classList.toggle('active',x===b));main.querySelectorAll('.progressView').forEach(x=>x.classList.toggle('active',x.dataset.progressView===id));};rail.appendChild(b);const view=views.get(id)||progressView(label,'This progress section is being prepared.');view.dataset.progressView=id;view.classList.toggle('active',id===activitiesProgressState.tab);main.appendChild(view);});
...
async function loadActivities(){
  ...
  try{
    ...
    if(window.D2Loader){D2Loader.update('Building Progress Workspace','Organizing activities, patterns, catalysts, triumphs, and collections…');D2Loader.progress(0.67);}
  ...
  }catch(err){body.innerHTML='';body.appendChild(el('div','fsState err','Could not load Activities & Progress: '+err.message));}
