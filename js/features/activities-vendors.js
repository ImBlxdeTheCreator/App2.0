...
function buildOperationCard(operation){
  ...
  {const img=document.createElement('img');D2Assets.setImage(img,operation,'activity',{alt:''});head.appendChild(img);}
  ...
  if(operation.rewardItemHashes.length){
    ...
    operation.rewardItemHashes.forEach(hash=>{
      ...
      getItemDefinition(hash).then(def=>{
        row.textContent='';{const img=document.createElement('img');D2Assets.setImage(img,def,def.itemType===2?'armor':'weapon',{alt:''});row.appendChild(img);}row.appendChild(el('span','',def.name));
      }).catch(()=>row.remove());
    });
...
const PROGRESS_TABS=[['overview','Overview'],['vanguard','Vanguard'],['crucible','Crucible'],['gambit','Gambit'],['rad','Raids & Dungeons'],['quests','Quests']];
...
async function resolveQuestCards(profile,limit=120){
  ...
  await Promise.all(entries.slice(0,500).map(async item=>{
    ...
    try{
      ...
      const liveBucket=Number(item.bucketHash||def?.inventory?.bucketTypeHash||def?.bucketHash||0);
      let bucketName='';
      try{const bucketDef=liveBucket?await getManifestEntity('DestinyInventoryBucketDefinition',liveBucket):null;bucketName=bucketDef?.displayProperties?.name||'';}catch(_){ }
      const text=`${bucketName} ${def?.itemTypeDisplayName||''} ${def?.itemTypeAndTierDisplayName||''} ${def?.displayProperties?.name||''} ${def?.displayProperties?.description||''}`;
      if(!window.D2InventoryRules?.isQuest({...item,_def:{itemTypeDisplayName:def?.itemTypeDisplayName,itemTypeAndTierDisplayName:def?.itemTypeAndTierDisplayName,name:def?.displayProperties?.name,description:def?.displayProperties?.description,bucketHash:liveBucket}},bucketName))return;
...
async function buildProgressViews(){
  ...
  if(profile){
    ...
    if(questCards.length){const sec=sectionBlock('Active Quests & Pursuits',`${questCards.length} found`),grid=el('div','progressCards');questCards.forEach(q=>{const card=progressDataCard(q.name,q.desc,0);if(q.icon){const img=document.createElement('img');D2Assets.setImage(img,q,'quest',{alt:''});card.prepend(img);}grid.appendChild(card);});sec.appendChild(grid);quests.appendChild(sec);}
  ...
  // Catalysts and weapon patterns belong inside Quests & Progress, not the Vault.
  const patternSection=sectionBlock('Weapon Patterns','Enclave-style crafting progress');
  if(profile){const craftables=profile.profileCraftables?.data?.craftables||{};const cards=await resolveCraftableCards(craftables);if(cards.length){const grid=el('div','progressCards');cards.forEach(c=>grid.appendChild(progressDataCard(c.name,c.desc,c.pct)));patternSection.appendChild(grid);}else patternSection.appendChild(el('div','progressEmpty','No pattern entries were returned for this profile.'));}else patternSection.appendChild(el('div','progressEmpty','Sign in with Bungie to load patterns.'));
  quests.appendChild(patternSection);
  const allQuestRecords={...(profile?.profileRecords?.data?.records||{})};Object.values(profile?.characterRecords?.data||{}).forEach(x=>Object.assign(allQuestRecords,x?.records||{}));
  const questRecordCards=profile?await resolveRecordCards(allQuestRecords):[];
  const catalystSection=sectionBlock('Catalysts','Exotic catalyst objectives and completion');const catalystCards=questRecordCards.filter(c=>/catalyst/i.test(`${c.name} ${c.desc}`));
  if(catalystCards.length){const grid=el('div','progressCards');catalystCards.forEach(c=>grid.appendChild(progressDataCard(c.name,c.complete?'Complete':c.desc||'In progress',c.pct)));catalystSection.appendChild(grid);}else catalystSection.appendChild(el('div','progressEmpty',profile?'No catalyst records were identified.':'Sign in with Bungie to load catalysts.'));
  quests.appendChild(catalystSection);views.set('quests',quests);
...
function buildActivityCard(c){
  ...
  const head=el('div','head');{const img=document.createElement('img');D2Assets.setImage(img,c,'activity',{alt:c.name});head.appendChild(img);}
...
function renderVendorCategoryItems(vendor,category,host){
  ...
  visible.forEach(item=>{
    ...
    {const im=document.createElement('img');im.className='ic';im.fetchPriority='low';D2Assets.setImage(im,def,def.itemType===2?'armor':'weapon',{alt:''});cell.appendChild(im);}
    ...
    (item.costs||[]).forEach(cost=>{
      ...
      if(cost.def){const ci=document.createElement('img');D2Assets.setImage(ci,cost.def,'currency',{alt:''});cr.appendChild(ci);}
...
function buildVendorSection(vendor){
  ...
  if(vendor.logo){const img=document.createElement('img');D2Assets.setImage(img,{name:vendor.name,icon:vendor.logo,hash:vendor.vhash},'vendor',{alt:`${vendor.profile.factionLabel} logo`});logoWrap.appendChild(img);}
  ...
  vendor.categories.forEach(category=>{
    ...
    if(category.icon){const icon=document.createElement('img');D2Assets.setImage(icon,{name:category.name,icon:category.icon},'vendor',{alt:''});summary.appendChild(icon);}
