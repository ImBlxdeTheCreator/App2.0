...
(function(){
  ...
  const overlay=document.getElementById('workspace-loadouts');
  ...
  function open(){ window.D2Workspaces?.activate?.('loadouts',{scroll:false});renderList();setTimeout(()=>nameInput?.focus?.(),40); }
  function close(){ window.D2Workspaces?.activate?.('builder',{scroll:false}); }
  function renderList(){
    const items=loadLibrary().sort((a,b)=>b.updatedAt-a.updatedAt);
    body.innerHTML='';
    if(!items.length){
      const e=document.createElement('div');e.className='loadoutEmpty';e.innerHTML='<strong>No loadouts saved yet.</strong><br><br>Make one and save it here, Guardian.';body.appendChild(e);return;
    }
    items.forEach(item=>{
