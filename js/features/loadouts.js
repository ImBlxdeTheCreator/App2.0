...
(function(){
  ...
  function renderList(){
    ...
    items.forEach(item=>{
      ...
      const overwrite=document.createElement('button');overwrite.textContent='Update';overwrite.onclick=()=>{item.state=snapshotState();item.updatedAt=Date.now();item.source=getStoredAuth&&getStoredAuth()?'bungie':'offline';const all=loadLibrary().map(x=>x.id===item.id?item:x);saveLibrary(all);renderList();};
  ...
  document.getElementById('loadoutsBtn').onclick=open;
