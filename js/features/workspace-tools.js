...
(function(){
  ...
  function renderCollections(panel){
    ...
    function draw(){
      ...
      grid.innerHTML=filtered.slice(0,240).map(item=>{const own=ownership(item),icon=iconOf(item);return `<button class="collectionCard ${own?'owned':'unowned'}" type="button"><span class="collectionImage">${icon?`<img loading="lazy" decoding="async" src="${esc(icon.startsWith('http')?icon:'https://www.bungie.net'+icon)}" alt="">`:''}</span><strong>${esc(nameOf(item))}</strong><span>${own?'Owned':'Not currently owned'}</span></button>`;}).join('')||'<p class="workspaceMuted">No matching definitions.</p>';
    }
