...
function fsIsQuestOrPursuitItem(it){
  const bucket=fsBucketHashForItem(it),def=it?._def||{};
  const text=`${fsBucketInfo(bucket).name||''} ${fsBucketLabel(it)||''} ${def.typeName||''} ${def.itemTypeDisplayName||''} ${def.itemTypeAndTierDisplayName||''} ${def.name||''}`.toLowerCase();
  return bucket===FS_QUEST_BUCKET || /(^|\b)(quest|quests|bounty|bounties|pursuit|pursuits|mission|missions)(\b|$)/.test(text);
}
function fsIsExcludedPresentationItem(it){
  const bucket=fsBucketHashForItem(it),def=it?._def||{};
  if(FS_HIDDEN_BUCKETS.has(bucket)||fsIsQuestOrPursuitItem(it))return true;
