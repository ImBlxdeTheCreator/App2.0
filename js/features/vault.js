...
function renderFsBody(){
  ...
  fsState.stores.forEach(store=>{
    ...
    const byBucket={};
    store.items.forEach(it=>{ if(fsItemMatchesFilter(it)){ let h=fsBucketHashForItem(it); if(FS_DESTINATION_CURRENCY_BUCKETS.has(h))h='destination-currency'; (byBucket[h]=byBucket[h]||[]).push(it); } });
    let shown=0;
    Object.keys(byBucket).sort(fsBucketSort).forEach(hash=>{
      const items=byBucket[hash]; if(!items?.length) return; shown+=items.length;
      const info=hash==='destination-currency'?{name:'Destination Currency'}:fsBucketInfo(hash); const sec=el('section','fsBucket');
