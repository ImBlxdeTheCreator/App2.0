...
const FS_RULES=window.D2InventoryRules;
const FS_POSTMASTER_BUCKET=FS_RULES.BUCKETS.POSTMASTER;
const FS_QUEST_BUCKET=FS_RULES.BUCKETS.QUESTS;
const FS_CLAN_BANNER_BUCKET=FS_RULES.BUCKETS.CLAN_BANNER;
const FS_DESTINATION_CURRENCY_BUCKETS=new Set(FS_RULES.BUCKETS.DESTINATION_CURRENCY);
const FS_HIDDEN_BUCKETS=new Set(FS_RULES.BUCKETS.HIDDEN);
...
function fsIsQuestOrPursuitItem(it){return FS_RULES.isQuest(it,fsBucketInfo(fsBucketHashForItem(it)).name||'');}
function fsIsExcludedPresentationItem(it){return FS_RULES.excludedFromVault(it,fsBucketInfo(fsBucketHashForItem(it)).name||'');}
...
function openClanBannerDetail(entries,chars){
  ...
  const bannerEntry=entries.find(e=>fsClanEntryRole(e)==='Clan Banner')||entries[0];
  const banner=bannerEntry?.item?._def||{name:'Clan Banner'};
  const genericEngrams=entries.filter(e=>fsClanEntryRole(e)==='Clan Engram');
  ...
  const hero=el('header','dimHero');
  const iconFrame=el('div','dimItemIconFrame');const icon=document.createElement('img');D2Assets.setImage(icon,banner,'emblem',{alt:'Clan Banner',lazy:false});iconFrame.appendChild(icon);hero.appendChild(iconFrame);
  const copy=el('div','dimHeroCopy');copy.appendChild(el('h2','dimItemName','Clan Banner'));copy.appendChild(el('div','dimItemMeta','Clan level, weekly activity engrams, and upgrades'));hero.appendChild(copy);
  ...
  const engramSec=el('section','dimSection');engramSec.appendChild(el('div','dimSectionTitle','Weekly Clan Engrams'));
  const engramGrid=el('div','fsClanActivityGrid');
  const expected=[
    {role:'Nightfall',aliases:/nightfall|vanguard/i},
    {role:'Gambit',aliases:/gambit/i},
    {role:'Raid',aliases:/raid/i},
    {role:'Crucible',aliases:/crucible|pvp/i},
  ];
  expected.forEach((spec,index)=>{
    const roleEntry=entries.find(e=>spec.aliases.test(`${e.item?._def?.name||''} ${e.item?._def?.typeName||''}`));
    const rewardEntry=roleEntry||genericEngrams[index]||genericEngrams[0]||null;
    const cardEl=el('article','fsClanActivityCard');
    const activity=el('div','fsClanActivitySource');
    const activityIcon=document.createElement('img');
    if(roleEntry)D2Assets.setImage(activityIcon,roleEntry.item._def,'activity',{alt:spec.role,lazy:false});
    else D2Assets.applyFallback(activityIcon,'activity');
    activity.appendChild(activityIcon);activity.appendChild(el('strong','',spec.role));
    const arrow=el('span','fsClanRewardArrow','→');
    const reward=el('div','fsClanActivityReward');
    const rewardIcon=document.createElement('img');
    if(rewardEntry)D2Assets.setImage(rewardIcon,rewardEntry.item._def,'engram',{alt:`${spec.role} Clan Engram`,lazy:false});
    else D2Assets.applyFallback(rewardIcon,'engram');
    reward.appendChild(rewardIcon);reward.appendChild(el('span','',rewardEntry?'Clan Engram':'Not returned'));
    cardEl.append(activity,arrow,reward);engramGrid.appendChild(cardEl);
  });
...
function fsAssetTypeForItem(it){return FS_RULES.assetType(it,fsBucketInfo(fsBucketHashForItem(it)).name||'');}
