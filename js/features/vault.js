...
const FS_DESTINATION_CURRENCY_BUCKETS = new Set([370330657,3703306568,2207872501]);
const FS_HIDDEN_BUCKETS = new Set([1753109658,2422292810,3621873013,444348033]);
...
function fsItemMatchesFilter(it){
  ...
  if(FS_HIDDEN_BUCKETS.has(bucket)||bucket===FS_QUEST_BUCKET) return false;
...
function buildFsPrioritySection(title,entries,chars,options={}){
  ...
  label.appendChild(el('span','fsBucketCount',options.countText||String(entries.length)));
  sec.appendChild(label);
  if(options.note)sec.appendChild(el('div','fsPostmasterNote',options.note));
  const grid=el('div','fsItems');
  entries.forEach(({item,store})=>grid.appendChild(buildFsItemTile(item,store,chars)));
...
function buildClanBannerSection(entries,chars){
  ...
  sec.appendChild(el('div','fsPostmasterNote','Clan Banner rewards and Clan Engrams are grouped here. Select an item to inspect its live Bungie definition and reward state.'));
  const grid=el('div','fsItems fsClanBannerItems');
  entries.forEach(({item,store})=>grid.appendChild(buildFsItemTile(item,store,chars)));
  if(!entries.length)grid.appendChild(el('div','fsEmpty','No Clan Banner or Clan Engram data was returned for this account.'));
  sec.appendChild(grid);return sec;
}
...
function renderFsBody(){
  ...
  const postmasterEntries=[];
  chars.forEach(store=>{
    const pmStore={kind:'postmaster',charId:store.charId,classType:store.classType,items:store.postmasterItems||[]};
    (store.postmasterItems||[]).filter(fsItemMatchesFilter).sort(fsItemSort).forEach(item=>{postmasterEntries.push({item,store:pmStore});claimed.add(fsItemIdentity(item));});
  });
  priorityWrap.appendChild(buildFsPrioritySection('Postmaster',postmasterEntries,chars,{className:'fsPostmasterSection',labelClass:'fsPostmasterLabel',countText:`${postmasterEntries.length}/${FS_POSTMASTER_CAPACITY_FALLBACK*Math.max(chars.length,1)}`,emptyText:'Postmaster is empty.',note:'Postmaster is always shown first. Lost Items remain separate from character and Vault capacity.'}));
  const engrams=[],orders=[],clan=[];
  fsState.stores.forEach(store=>(store.items||[]).forEach(item=>{
    if(!fsItemMatchesFilter(item))return;
    const kind=fsPriorityBucketKind(item);if(!kind)return;
    const entry={item,store};claimed.add(fsItemIdentity(item));
    if(kind==='engram')engrams.push(entry);else if(kind==='order')orders.push(entry);else clan.push(entry);
  }));
  priorityWrap.appendChild(buildFsPrioritySection('Engrams',engrams,chars,{emptyText:'No engrams are currently held.'}));
  priorityWrap.appendChild(buildFsPrioritySection('Orders',orders,chars,{emptyText:'No order items are currently held.'}));
...
function buildFsItemTile(it,store,chars){
  ...
  {const img=document.createElement('img');img.fetchPriority='low';D2Assets.setImage(img,def,def.itemType===2?'armor':'weapon',{alt:def.name});tile.appendChild(img);}
